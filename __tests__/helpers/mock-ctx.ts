...
export function createMockCtx(extras?: MockCtxExtras): MockCtx {
...
export async function tick(rounds = 5): Promise<void> {
  for (let i = 0; i < rounds; i++) await Bun.sleep(0);
}

/** Minimal v2 tool execute.before event. */
export function beforeEvent(tool: string, input: unknown): Record<string, unknown> {
  return { tool, sessionID: "ses-test", agent: "build", messageID: "msg-1", id: "call-1", input };
}

/** Minimal v2 tool execute.after event. */
export function afterEvent(tool: string, input: unknown): Record<string, unknown> {
  return {
    tool,
    sessionID: "ses-test",
    agent: "build",
